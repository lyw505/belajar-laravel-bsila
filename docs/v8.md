...
Tujuan kali ini adalah menampilkan data dari tabel siswa **tanpa reload halaman (melalui Ajax)**.

---

## Langkah-langkah

### 1. Menampilkan data dari tabel siswa melalui Ajax

#### Memodifikasi controller berikut
...
### 3. Tambahkan atribut id pada tabel

Tambahkan `id="tabel-siswa"` di dalam tag `<table>` yang menampilkan data siswa.
...
## Menambahkan Fitur Pencarian Otomatis

### 1. Tambahkan input pencarian di atas tabel siswa

```html
<p><label>Cari Siswa: </label><input type="text" id="search" placeholder="Ketik nama..."></p>
```

### 2. Tambahkan function `search` di dalam controller `siswa`
...
### 3. Tambahkan script pencarian di bawah `loadSiswa()`
